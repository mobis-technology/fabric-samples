...
function printHelp () {
  ...
  echo "    -c <channel name> - test network channel name (defaults to \"aipanchannel\")"
  ...
  echo "	addOrg3.sh up"
  echo "	addOrg3.sh up -c aipanchannel -s couchdb"
...
function Org3Up () {
  ...
  if [ "${DATABASE}" == "couchdb" ]; then
    IMAGE_TAG=latest docker-compose -f $COMPOSE_FILE_ORG3 -f $COMPOSE_FILE_COUCH_ORG3 up -d 2>&1
  else
    IMAGE_TAG=latest docker-compose -f $COMPOSE_FILE_ORG3 up -d 2>&1
  fi
  if [ $? -ne 0 ]; then
...
#default for delay
CLI_DELAY=3
# channel name defaults to "aipanchannel"
